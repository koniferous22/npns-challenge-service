...
ARG IMAGE_TAG_CONTENT_SERVICE_ALPINE
ARG NODE_ENV
FROM node:$IMAGE_TAG_CONTENT_SERVICE_ALPINE AS base_node_alpine
RUN echo "Building content service with NODE_ENV=\"${NODE_ENV}\""

WORKDIR /usr/src/content-service
COPY package.json package-lock.json ./
RUN npm ci 

# VERSION 1 of the image: npm start:dev
FROM base_node_alpine AS npns_production
ENV CONTENT_SERVICE_START_COMMAND=start
# Source code is copied and built in this image
COPY . /usr/src/content-service
RUN npm run build
